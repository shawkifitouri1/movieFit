...
import { searchMovies, clearSearchResults, AddToFavorites } from "../backend/redux/movies/movie.actions";
...
const Favorites = (props) => {
    ...
    const getFavoriteMovies = () => {
        return props.favoriteList.map((favoriteId) =>
            props.movies.find((movie) => movie.id === favoriteId)
        );
    };
...
console.log(props.favoriteList,props.movies.filter((movie) => !props.favoriteList.includes(movie.id)));
        ...
        <SafeAreaView style={styles.container}>
            ...
            >
                ...
                <Animated.View style={[{ flex: 6 }, { opacity: fadeAnim }]}>
                    <MovieGridSection  isFav={true} movies={props.movies.filter((movie) => props.favoriteList.includes(movie.id))}    searchResults={[]} loading={props.loading} />
                     </Animated.View>
            </ImageBackground>
        </SafeAreaView>
    );
